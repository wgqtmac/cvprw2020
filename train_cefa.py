...
from dataset.RGBImgLoader import RGBImgLoader
import roc
...
def generate_result(model, dataloader):
    data_write = open("./result.txt", 'a+')
    ...
    with torch.no_grad():
        for cnt, data in enumerate(dataloader, 0):
            ...
            newline = img + "{:.8f}".format(probability_value[0]) + "\n"
...
if __name__ == '__main__':
    ...
    parser.add_argument('-train_list', type=str, default='4@all_train.txt', help='initial learning rate')
    parser.add_argument('-test_list', type=str, default='4@all_dev.txt', help='initial learning rate')
    ...
    out_dir = './logs'
    log = Logger()
    # log.open(os.path.join(out_dir, args.net + 'CeFA@3.txt'), mode='a')
    log.open(os.path.join(out_dir, args.net + 'CASIA@4.txt'), mode='a')
    ...
    val_dataset = ImgLoader(args.root_folder, os.path.join(args.root_folder, args.test_list),
                              transforms.Compose([
                                  transforms.Resize(248),
                                  # transforms.RandomAffine(10),
                                  transforms.CenterCrop(248),
