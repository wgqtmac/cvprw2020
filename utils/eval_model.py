...
def eval_turn(Config, model, data_loader, val_version, epoch_num, log_file):
    ...
    with torch.no_grad():
        for batch_cnt_val, data_val in enumerate(data_loader):
            ...
            top1_val, top1_pos = torch.topk(outputs_pred, 1)
            ...
            batch_corrects1 = torch.sum((top1_pos[:, 0] == labels)).data.item()
            val_corrects1 += batch_corrects1
            for i in range(len(labels)):
                if labels[i] == 1 and top1_pos[:, 0][i] == 1:
                    TP += 1
                elif labels[i] == 0 and top1_pos[:, 0][i] == 0:
                    TN += 1
                elif labels[i] == 1 and top1_pos[:, 0][i] == 0:
                    FN += 1
                elif labels[i] == 0 and top1_pos[:, 0][i] == 1:
                    FP += 1
        ...
        print('TP:{} FN:{} TN:{} FP:{} TP rate:{}, TN rate:{}, HTER:{}'.format(TP, FN, TN, FP, float(TP / (TP + FN)), float(TN / (TN + FP)), HTER))
        ...
        log_file.write(val_version + '\t' + str(val_loss_recorder.get_val()) + '\t' + str(val_celoss_recorder.get_val()) + '\t' + str(val_acc1) + '\t' + str(HTER) + '\n')
        ...
        print('% 3d %s %s %s-loss: %.4f ||%s-acc@1: %.4f ||HTER: %.4f ||time: %d' % (epoch_num, val_version, dt(), val_version, val_loss_recorder.get_val(init=True), val_version, val_acc1, HTER, since), flush=True)
        print('--' * 30, flush=True)
