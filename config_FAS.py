...
class LoadConfig(object):
    def __init__(self, args, version):
        ...
        if args.dataset == 'product':
            ...
            self.numcls = 100
        elif args.dataset == 'FAS':
            self.dataset = args.dataset
            self.rawdata_root = '/home/gqwang/Spoof_Croped/CASIA_CeFA'
            self.anno_root = '/home/gqwang/Spoof_Croped/CASIA_CeFA'
            self.numcls = 2
        else:
        ...
        if 'train' in get_list:
             self.train_anno = pd.read_csv(os.path.join(self.anno_root, '4@1_train.txt'),\
                                           sep=" ",\
                                           header=None,\
                                           names=['ImageName', 'label'])

        if 'val' in get_list:
            self.val_anno = pd.read_csv(os.path.join(self.anno_root, '4@1_dev_list.txt'),\
                                           sep=" ",\
                                           header=None,\
                                            names=['ImageName', 'label'])

        if 'test' in get_list:
            self.test_anno = pd.read_csv(os.path.join(self.anno_root, '4@1_dev_list.txt'),\
                                           sep=" ",\
                                           header=None,\
                                           names=['ImageName', 'label'])
